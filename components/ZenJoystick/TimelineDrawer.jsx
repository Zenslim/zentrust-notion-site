      ...
      <div className="p-4 space-y-4 overflow-y-auto">
        ...
        {entries.map(entry => (
          <div key={entry.id} className="bg-zinc-800 p-3 rounded shadow hover:bg-zinc-700">
            <div className="text-sm text-zinc-400">{new Date(entry.timestamp?.toDate?.() || Date.now()).toLocaleString()}</div>
            <div className="text-xs mt-1">🕊 Spiritual: {entry.spiritual} | 🤝 Social: {entry.social} | 🧠 Psycho: {entry.psycho} | 🌞 Bio: {entry.bio}</div>
            <div className="mt-1 text-sm italic text-zinc-300">“{entry.note?.slice(0, 120) || "No thoughts"}”</div>
          </div>
        ))}
      </div>
    </div>
